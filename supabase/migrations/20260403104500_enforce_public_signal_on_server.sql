...
DECLARE
  ...
  v_signal_enabled boolean := false;
  v_signal_type text := 'percentage';
  v_signal_value numeric := 0;
  v_effective_signal_amount numeric(10,2) := 0;
BEGIN
  ...
  SELECT
    COALESCE(pc.signal_enabled, false)
      AND COALESCE(pc.accept_pix, false)
      AND nullif(trim(coalesce(pc.pix_key, '')), '') IS NOT NULL
      AND EXISTS (
        SELECT 1
    ...
    COALESCE(pc.signal_type, 'percentage'),
    COALESCE(pc.signal_value, 0)
  INTO v_signal_enabled, v_signal_type, v_signal_value
  FROM public.payment_config pc
  WHERE pc.professional_id = p_professional_id;

  IF v_signal_enabled THEN
    IF v_signal_type = 'percentage' THEN
      v_effective_signal_amount := ROUND((v_total_price * v_signal_value) / 100, 2);
    ELSE
      v_effective_signal_amount := LEAST(v_signal_value, v_total_price);
    END IF;

    IF COALESCE(v_effective_signal_amount, 0) <= 0 THEN
      v_signal_enabled := false;
      v_effective_signal_amount := 0;
    END IF;
  END IF;
  ...
  INSERT INTO public.bookings (
    ...
    p_start_time,
    v_end_time,
    CASE WHEN v_signal_enabled THEN 'pending' ELSE 'confirmed' END,
    v_total_price,
    v_total_duration,
    trim(p_client_name),
    v_client_phone,
    CASE WHEN v_signal_enabled THEN v_effective_signal_amount ELSE 0 END,
    CASE WHEN v_signal_enabled THEN now() + interval '5 minutes' ELSE NULL END
  )
  ...
  RETURN json_build_object(
    ...
    'end_time', v_end_time,
    'status', CASE WHEN v_signal_enabled THEN 'pending' ELSE 'confirmed' END,
    'signal_amount', CASE WHEN v_signal_enabled THEN v_effective_signal_amount ELSE 0 END
  );
END;
$$;
