...
const OPENAI_BASE_URL = (Deno.env.get("OPENAI_API_BASE_URL") || "https://api.openai.com/v1").replace(/\/$/, "");
...
export async function callProvider(input: AIProviderCallInput): Promise<AIProviderCallSuccess> {
  ...
  const timeoutMs = input.timeoutMs ?? 18000;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw makeError({
        message: `OPENAI_HTTP_${response.status}`,
        model: input.model,
    ...
    if ((error as AIProviderCallError)?.provider === "openai") throw error;
